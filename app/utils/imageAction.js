...
export const getPickedData = (pickedData, existingImageLength) => {
  if (
    pickedData?.length >
    MAX_UPLOAD_POST_ASSETS - (existingImageLength ?? 0)
  ) {
    return pickedData.slice(
      0,
      MAX_UPLOAD_POST_ASSETS - (existingImageLength ?? 0),
    );
  }
...
export const thumbnailImageSize = (imageData) => {
  ...
  console.log('IIIIIIIIMMMMMMMMMMGGGGG', img);
  if (imageData.height < imageData.width) {
    if (imageData.height > T_LANDSCAPE_IMAGE_HEIGHT) {
      img.height = T_LANDSCAPE_IMAGE_HEIGHT;
      img.width = parseInt(
        (imageData.width / imageData.height) * T_LANDSCAPE_IMAGE_HEIGHT,
        10,
      );
    } else {
  ...
  if (imageData.width > T_PORTRAIT_IMAGE_WIDTH) {
    img.height = parseInt(
      (imageData.height / imageData.width) * T_PORTRAIT_IMAGE_WIDTH,
      10,
    );
...
const uploadImage = async (data, authContext, cancelToken, preSignedUrls) => {
  ...
  if (image?.mime?.split('/')?.[0] === 'image') {
    ...
    resThumb = await ImageResizer.createResizedImage(
      image.path,
      thumbImgData.width,
      thumbImgData.height,
      'JPEG',
    ...
    resOriginal = await ImageResizer.createResizedImage(
      image.path,
  ...
  const promises = [
    uploadImageOnPreSignedUrls({
      url: preSignedUrls[0],
      uri:
        image?.mime?.split('/')?.[0] === 'image' ? resOriginal.uri : image.path,
      type: image.mime,
      cancelToken,
    }),
    uploadImageOnPreSignedUrls({
...
const uploadImages = async (
  ...
  new Promise((resolve, reject) => {
    ...
    getImagePreSignedURL(
      ...
      const preSignedUrls = await _.chunk(
        responsePresignedURLS?.payload?.preSignedUrls,
        2,
      );
