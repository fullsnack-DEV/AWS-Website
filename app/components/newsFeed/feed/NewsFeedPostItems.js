...
const NewsFeedPostItems = memo(({currentParentIndex, parentIndex, navigation, item = {}, onLikePress, caller_id, onDeletePost, onImageProfilePress, onEditPressDone, updateCommentCount, isNewsFeedScreen, openProfilId, entityDetails = {}, fetchFeeds = () => {}}) => {
  ...
  const handleFollowUnfollow = (userId, isFollowing = false, entityType = Verbs.entityTypePlayer) => {
    const params = {
    ...
    <View style={{paddingHorizontal: 15, paddingTop: 17, paddingBottom: 20}}>
      {postType === Verbs.eventVerb ? (
        <PostForEvent
          ...
          showMoreOptions={authContext.entity.role === Verbs.entityTypePlayer || authContext.entity.role === Verbs.entityTypeUser}
        />
      ) : (
        <Post
          item={item}
          onImageProfilePress={onImageProfilePress}
          onThreeDotPress={() => {
            const tagUser = item.tagged.find((ele) => ele.id === authContext.entity.uid);

            let option = [];
            if (item.actor.id === authContext.entity.uid) {
              option = [strings.edit, strings.delete];
            } else if (tagUser?.id) {
              option = [strings.removeMyTagFromPost, strings.report, strings.blockUser];
            } else {
      ...
      />
      <BottomSheet type="ios" isVisible={showMoreOptions} closeModal={() => setShowMoreOptions(false)} optionList={moreOptions} onSelect={(value) => onActionSheetItemPress(value)} />

      <BottomSheet type="ios" isVisible={showShareOptionsModal} closeModal={() => setShowShareOptionsModal(false)} optionList={[strings.repost, strings.copyLink, strings.more]} onSelect={onShareActionSheetItemPress} />
    </View>
  );
});
