...
const ProfileView = ({ data }) => {
    ...
    useEffect(() => {
        if (data?.length > 0) {
            const imgArray = data.slice(0, 2).map((item) => (JSON.parse(item.customData)))
    ...
    if (occupantData?.[0]?.entity_type === 'team') entity_small_icon_image = images.teamSmallIcon
    if (occupantData?.[0]?.entity_type === 'club') entity_small_icon_image = images.clubSmallIcon
    if (occupantData?.[0]?.entity_type === 'league') entity_small_icon_image = images.leagueSmallIcon
...
const ProfilePreview = ({ data, isSingle = false }) => {
    ...
    return (
      <View style={{ ...styles.profilePreViewContainer, height: outerCircleSize, width: outerCircleSize }}>
        <View style={{ ...styles.imageContainer, height: innerCircleSize, width: innerCircleSize }}>
          <FastImage
                    resizeMode={'contain'}
          ...
          {display_first_character ? (
            <View style={styles.display_first_characterContainer}>
              <Text style={styles.display_first_character}>{data?.full_name?.[0]}</Text>
            </View>
        ) : null}
