...
function TCRefereeView({onPress, showStar = false, data, sport}) {
  const authContext = useContext(AuthContext);

  let sportObj = data?.referee_data?.filter((o) => o?.sport === sport?.sport);
  if (sportObj.length === 1) {
    sportObj = data?.referee_data?.filter((o) => o?.sport === sport?.sport);
  } else {
