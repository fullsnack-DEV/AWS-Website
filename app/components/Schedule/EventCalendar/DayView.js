...
function range(from, to) {
  return Array.from(Array(to), (i) => from + i)
}
...
export default class DayView extends React.PureComponent {
  ...
  scrollToFirst() {
    setTimeout(() => {
      if (this.state && this.state.scrollY && this.scrollView) {
        this.scrollView.scrollTo({ x: 0, y: this.state.scrollY, animated: true })
      }
    }, 1)
  }
  ...
  renderTimeLabels() {
    ...
    const offset = CALENDER_HEIGHT / 24
    return range(0, 24).map((item, i) => <View key={`line${i}`} style={[styles.line, { top: offset * i }]} />)
  }
