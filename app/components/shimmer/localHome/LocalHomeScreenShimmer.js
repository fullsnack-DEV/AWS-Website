    ...
    <View>
      ...
      <ScrollView horizontal={true} showsHorizontalScrollIndicator={false} showsVerticalScrollIndicator={false}>
        {new Array(5).fill('').map((childItem, childIndex) => (
          <View key={`child${childIndex}`} style={{ paddingHorizontal: 15, marginVertical: 10 }}>
            <ShimmerView width={100} height={15} style={{ alignSelf: 'flex-start' }}/>
          </View>
        ))}
      </ScrollView>
      <ShimmerSeperator style={{ marginVertical: 0 }} color={colors.graySeparater}/>

      {/*  Shorts Round List */}
      <ScrollView horizontal={true} showsHorizontalScrollIndicator={false} showsVerticalScrollIndicator={false}>
        {new Array(5).fill('').map((childItem, childIndex) => (
          <View key={`child${childIndex}`} style={{ paddingHorizontal: 10, marginVertical: 10 }}>
            <View style={{
                alignItems: 'center',
                justifyContent: 'center',
                height: 70,
      ...
      <View style={{
        ...
        <ShimmerView width={150} height={25} style={{ marginHorizontal: 15, alignSelf: 'flex-start' }}/>
        <ScrollView horizontal={true} showsHorizontalScrollIndicator={false} showsVerticalScrollIndicator={false}>
          {new Array(10).fill('').map((childItem, childIndex) => (
            <View key={`child${childIndex}`} style={{ paddingHorizontal: 7.5, marginVertical: 10 }}>
              <ShimmerView width={150} height={250} style={{ borderRadius: 15 }}/>
            </View>
                ))}
        </ScrollView>
      </View>
