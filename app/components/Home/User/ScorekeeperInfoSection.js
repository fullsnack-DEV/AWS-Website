...
function ScorekeeperInfoSection({
  ...
  const [certificatesData, setCertificatesData] = useState(
    selectScorekeeperData.certificates ?? [],
  );
  const [refereeFeeCount, setRefereeFeeCount] = useState(
    selectScorekeeperData.fee || 0,
  );
      ...
      <View
        ...
        {scorekeeperSetting?.available_area ? (
          scorekeeperSetting?.available_area?.is_specific_address ? (
            <FlatList
              data={
                scorekeeperSetting?.available_area?.address_list?.length > 0
                  ? scorekeeperSetting?.available_area?.address_list
                  : []
              }
              bounces={false}
              ListEmptyComponent={
