...
function ScorekeeperInfoSection({
  data,
  selectScorekeeperData,
  languagesName,
  onSavePress,
}) {
  ...
  const [bioText, setBioText] = useState(selectScorekeeperData.descriptions);
  const [certificatesData, setCertificatesData] = useState(selectScorekeeperData.certificates ?? []);
  const [refereeFeeCount, setRefereeFeeCount] = useState(selectScorekeeperData.fee || 0);
  ...
  const [selected, setSelected] = useState(() => {
    if (selectScorekeeperData.cancellation_policy === 'flexible') {
      return 2;
    }
    if (selectScorekeeperData.cancellation_policy === 'moderate') {
      return 1;
    }
    if (selectScorekeeperData.cancellation_policy === 'moderate') {
      return 0;
  ...
  const [selectedLanguage, setSelectedLanguage] = useState(() => {
    if (selectScorekeeperData.language && selectScorekeeperData.language.length > 0) {
      languageList.map((listItem) => {
        const listValue = listItem;
        selectScorekeeperData.language.map((default_lang) => {
  ...
  const onTopEditSavePress = (certiData) => {
    ...
    data.scorekeeper_data.forEach((item) => {
      if (item.sport_name?.toLowerCase() === selectScorekeeperData.sport_name?.toLowerCase()) {
...
export default ScorekeeperInfoSection;
