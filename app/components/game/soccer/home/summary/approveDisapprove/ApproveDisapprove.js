...
const MESSAGE =
  'If the both teams don’t approved the winner and scores before the match-result-approving period, the game will be a RDG (Result-Disapproved-game). The total number of RDGs of each team will be displayed in its Stats.';
...
const ApproveDisapprove = ({
  ...
  const onApproveDisapprovePress = (type = GameRecordStatus.Approve) => {
    const myTeamNumber =
      approvalGameData?.home_team?.group_id === myTeamId ? 1 : 2;
    setLoadingTeam(myTeamNumber);
    approveDisapproveGameScore(gameId, myTeamId, type, {
  ...
  const isApprovingPeriodExpired =
    new Date() > new Date(approvalGameData?.approval?.expiry * 1000);
        ...
        <Fragment>
          {isButtonShown && !isApproved ? (
            <Fragment>
              <TCGradientButton
                onPress={() =>
                  onApproveDisapprovePress(GameRecordStatus.Approve)
                }
              ...
              <TCGradientButton
                onPress={() =>
                  onApproveDisapprovePress(GameRecordStatus.Disapprove)
                }
            ...
            <View style={{...styles.approveDisapproveSection, flex: 0.5}}>
              <Text
                style={{
                  color:
                    status === APPROVE_GAME ? '#FF8A01' : colors.googleColor,
                  fontSize: 16,
    ...
    <View style={styles.mainContainer}>
      ...
      <View style={styles.contentContainer}>
        <View style={{padding: 10}}>
          <Text style={styles.title}>Match Result Approval</Text>
          ...
          <View style={styles.teamVSContainer}>
            <View style={styles.teamVSContentContainer}>
              <View style={styles.teamLogoAndNameContainer}>
                <View style={styles.teamLogoContainer}>
                  <FastImage
                    ...
                    source={
                      approvalGameData?.home_team?.background_thumbnail
                        ? {
                            uri: approvalGameData?.home_team
                ...
                <Text style={styles.teamNameText}>
                  {approvalGameData?.home_team?.group_name ??
                    approvalGameData?.home_team?.full_name ??
                    ''}
                </Text>
              </View>
              {/* Home Team */}
              <ApproveDisapproveSection
                ...
                isApproved={
                  [true, false].includes(
                    approvalGameData?.approval?.home_team?.approved,
                  ) ?? false
                }
                isButtonShown={
                  !isApprovingPeriodExpired &&
                  (approvalGameData?.home_team?.group_id === myTeamId ||
                    approvalGameData?.home_team?.user_id === myTeamId)
                }
            ...
            <View style={styles.teamVSContentContainer}>
              <View style={styles.teamLogoAndNameContainer}>
                <View style={styles.teamLogoContainer}>
                  <FastImage
                    style={styles.teamLogo}
                    source={
                      approvalGameData?.away_team?.background_thumbnail
                        ? {
                            uri: approvalGameData?.away_team
                              ?.background_thumbnail,
                          }
                        : images.teamPlaceholder
                    }
                ...
                <Text style={styles.teamNameText}>
                  {approvalGameData?.away_team?.group_name ??
                    approvalGameData?.away_team?.full_name ??
                    ''}
                </Text>
              </View>
              {/* Away Team */}
              <ApproveDisapproveSection
                ...
                isApproved={
                  [true, false].includes(
                    approvalGameData?.approval?.away_team?.approved,
                  ) ?? false
                }
                isButtonShown={
                  !isApprovingPeriodExpired &&
                  (approvalGameData?.away_team?.group_id === myTeamId ||
                    approvalGameData?.away_team?.user_id === myTeamId)
                }
      ...
      {isApprovingPeriodExpired &&
        approvalGameData?.approval?.status !== 'approvedByAll' && (
          <Text style={styles.reviewPeriod}>
            The match-result-approving period is
            <Text style={{fontFamily: fonts.RBold}}> expired</Text>
          </Text>
        )}
      {!isApprovingPeriodExpired && (
        <Text style={styles.reviewPeriod}>
          The match-result-approving period will be expired with{' '}
          <Text style={{fontFamily: fonts.RBold}}>
            {approvalGameData?.approval?.expiry &&
              getGameDateTimeInDHMformat(approvalGameData?.approval?.expiry)}
          </Text>
        </Text>
      )}
    </View>
