...
import NewsFeedDescription from '../../../../../newsFeed/NewsFeedDescription';
   ...
   function RefereeFeedPostItems({
     ...
     useEffect(() => {
      console.log('Referee Review', item);
    }, [])
     ...
     let descriptions = '';
     if (item?.object) {
       if (JSON.parse(item?.object)?.refereeReview?.attachments !== undefined && JSON.parse(item?.object)?.refereeReview?.attachments?.length > 0) {
