...
const MatchRecordsList = ({gameData, gameId = null, getGameMatchRecords}) => {
  ...
  const renderGameRecords = useCallback(
    ({item}) => {
      ...
      return (
        <View>
          {!isGameState && isHomeTeam && (
            <View>
              <RenderDash zIndex={1} />
              <TennisGameScoreLeft gameData={gameData} recordData={item} />
            </View>
          )}
          {!isGameState && !isHomeTeam && (
            <View>
              <RenderDash zIndex={1} />
              <TennisGameScoreRight gameData={gameData} recordData={item} />
            </View>
          )}
  ...
  const ListEmptyComponent = useMemo(
    () => (
      <View>
        <Text style={styles.notAvailableTextStyle}>Not available yet</Text>
      </View>
    ),
    [],
  );

  const matchRecordsKeyExtractor = useCallback(
  ...
  return (
    <View style={styles.mainContainer}>
      ...
      <View style={styles.seperateContainer}></View>
      <Text style={styles.listFooterText}>
        This game record hasn’t yet been confirmed by teams.{' '}
      </Text>
    </View>
  );
};
