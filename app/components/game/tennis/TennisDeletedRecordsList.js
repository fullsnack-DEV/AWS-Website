...
const SwipeableRowButtons = [{ key: 'revert', fillColor: colors.themeColor, image: images.deleteIcon }]
export default function TennisDeletedRecordsList({
  matchData,
  isAdmin,
}) {
  ...
  const loadAtOnce = async () => {
    ...
    if (gameId) {
      ...
      getGameData(gameId, true, authContext).then(async (res) => {
        ...
        getGameMatchRecords(gameId, authContext, 'status=deleted&grouped=true').then((matchRes) => {
  ...
  const refreshGameAndRecords = () => {
    getGameData(gameData?.game_id, true, authContext).then(async (res) => {
      ...
      getGameMatchRecords(gameData?.game_id, authContext, 'status=deleted&grouped=true').then((matchRes) => {
        setMatchRecords([...matchRes.payload]);
        setLoading(false);
