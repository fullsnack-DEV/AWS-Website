...
export default function ScorekeeperAgreementView({
  teamA,
  teamB,
  type = 'challenge',
  numberOfScorekeeper,
  ...
  const rule1 = (
    <>
      ...
      {numberOfScorekeeper} scorekeepers for the game at its own cost within 5
      days after the challenge is accepted.
    </>
  );
  const rules2 = (
    <>
      • The consent of <Text style={styles.boldText}>{teamA}</Text> will be
      required when {teamB} books a specific scorekeeper.
    </>
  );
  const radioTitle = (
    <>
      If <Text style={styles.boldText}>{teamB}</Text> isn’t reserving{' '}
      {numberOfScorekeeper} scorekeepers for the game 5 days after the challenge
      is accepted,
    </>
  );
  const radio1 = (
  ...
  const radio2 = (
    <>
      ...
      than {numberOfScorekeeper} scorekeepers.
    </>
  );
