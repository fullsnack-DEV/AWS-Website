...
export default function PayoutMethodScreen({navigation}) {
    ...
    Utility.getStorage('appSetting').then((setting) => {
      merchantAuthDetail(authContext.entity.uid, authContext)
        .then((response) => {
          ...
          let urlString = `${setting.resgisterMerchantURL}?client_id=${
            response.payload.client_id
          }&state=${response.payload.state}&redirect_uri=${encodeURI(
            response.payload.redirect_uri,
          )}`;
          urlString = `${urlString}&stripe_user[business_type]=individual&stripe_user[business_name]=TownsCup&`;
          urlString = `${urlString}&stripe_user[first_name]=${encodeURI(
            authContext.entity.obj.group_name ||
              authContext.entity.obj.first_name,
          )}`;
          urlString = `${urlString}&stripe_user[last_name]=${encodeURI(
            authContext.entity.obj.last_name || '',
          )}`;
          urlString = `${urlString}&stripe_user[email]=${encodeURIComponent(
            authContext.entity.role === ('team' || 'club')
              ? authContext.entity.auth.user.email
  ...
  const renderLoadingView = () => <ActivityLoader visible={loading} />;
  return (
