...
export default function RecruitingMemberScreen({navigation, route}) {
  ...
  const [hiringPlayersSelection, setHiringPlayersSelection] = useState(
    (route?.params?.settingObj?.hiringPlayers === 'Yes' &&
      hiringPlayersOptions[0]) ||
      (route?.params?.settingObj?.hiringPlayers === 'No' &&
        hiringPlayersOptions[1]),
  );
  ...
  const saveTeam = () => {
    const bodyParams = {
      hiringPlayers:
        (hiringPlayersSelection.key === strings.yes && 'Yes') ||
        (hiringPlayersSelection.key === strings.no && 'No'),
    };
    setloading(true);
