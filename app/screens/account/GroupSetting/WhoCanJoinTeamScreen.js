...
const whoCanJoinTeamOpetions = [
  {key: strings.everyoneRadio, id: 1},
  {key: strings.personWhoRequest, id: 2},
...
export default function WhoCanJoinTeamScreen({navigation, route}) {
  ...
  const saveTeam = () => {
    const bodyParams = {
      who_can_join:
        (whoCanJoinTeam.key === strings.everyoneRadio && 0) ||
        (whoCanJoinTeam.key === strings.personWhoRequest && 1) ||
        (whoCanJoinTeam.key === strings.inviteOnly && 2),
    };
    setloading(true);
