...
export const getChallengeDetail = (challengeID, authContext) => {
  ...
  return getChallenge(challengeID, authContext)
    .then((response) => {
      ...
      if (response.payload.length > 0) {
        ...
        if (
          ReservationStatus.changeRequest === challenges[0].status ||
        ...
        if (
          ReservationStatus.pendingpayment === challenges[0].status ||
        ...
        if (
          ReservationStatus.restored === challenges[0].status ||
          ReservationStatus.requestcancelled === challenges[0].status
        ) {
          // let tempObj;
