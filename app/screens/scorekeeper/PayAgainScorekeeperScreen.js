...
import { payAgainAlterScorekeeper, payAgainScorekeeper } from '../../api/Challenge';
...
export default function PayAgainScorekeeperScreen({ navigation, route }) {
  ...
  useEffect(() => {
    if (isFocused) {
      ...
      console.log('Body Object of pay again scorekeeper screen: ', JSON.stringify(body));
      // getFeeDetail();
  ...
  const payAgainForScorekeeperRequest = () => {
    ...
    if (defaultCard) {
      ...
      if (sorceScreen === ReservationStatus.pendingrequestpayment) {
        payAgainAlterScorekeeper(reservationObj.reservation_id, bodyParams, authContext).then(() => {
          setloading(false)
