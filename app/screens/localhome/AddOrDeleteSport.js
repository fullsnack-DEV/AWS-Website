...
import { widthPercentageToDP } from '../../utils';
  ...
  export default function AddOrDeleteSport({ navigation, route }) {
    ...
    }, [navigation, sportsSource, selectedSports, systemSports, defaultSportsList]);
            ...
            if (res.payload) {
              ...
              for (const tempData of res.payload) {
                const isFound = sportsSource.filter(
                  (obj) => obj.sport_name.toLowerCase()
                    === tempData.sport_name.toLowerCase() && tempData.sport_name.toLowerCase() !== 'All',
                );
                if (isFound.length > 0) {
    ...
    const searchSportsFunction = (text) => {
        const result = systemSports.filter(
          (x) => x.sport_name.toLowerCase().includes(text.toLowerCase())
            || x.sport_name.toLowerCase().includes(text.toLowerCase()),
        );
        if (text.length > 0) {
    ...
    return (
      <SafeAreaView style={{ flex: 1 }}>
        ...
        <View style={ { height: '80%' }}>
          ...
          <SportsListView sports={systemSports} onSelect={isIconCheckedOrNot} defaultSport={defaultSportsList}/>
        </View>
      </SafeAreaView>
    );
  }
