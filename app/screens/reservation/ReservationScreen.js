...
export default function ReservationScreen({navigation}) {
  ...
  const getReservationListByCaller = () => {
    getReservationList(
      authContext.entity.role === 'team' && authContext.entity.uid,
      authContext,
    )
      .then((response) => {
        ...
        for (const temp of response.payload) {
          ...
          if (
            curruentDate < date &&
            temp.status !== ReservationStatus.completed &&
            temp?.game?.status !== GameStatus.ended
          ) {
        ...
        setUpcoming(
          upcomingData.sort((x, y) => x.start_datetime - y.start_datetime),
        );
