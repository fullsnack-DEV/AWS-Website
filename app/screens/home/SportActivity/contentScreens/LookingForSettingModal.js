...
const LookingForSettingModal = ({
  ...
  useEffect(() => {
    if (sportObj?.sport) {
      setLookingFor(
        sportObj.lookingForTeamClub
          ? strings.yesDisplayItText
          : strings.noDisplayItText,
      );
    }
  }, [sportObj?.sport, sportObj?.lookingForTeamClub]);
  ...
  const updateProfile = useCallback(() => {
    ...
    if (entityType === Verbs.entityTypeReferee) {
      body = {referee_data: sportList};
    }
    if (entityType === Verbs.entityTypeScorekeeper) {
      body = {scorekeeper_data: sportList};
    }
    if (entityType === Verbs.entityTypePlayer) {
      body = {registered_sports: sportList};
    }
  ...
  }, [authContext, closeModal, entityType, lookingFor, sportObj]);
...
export default LookingForSettingModal;
