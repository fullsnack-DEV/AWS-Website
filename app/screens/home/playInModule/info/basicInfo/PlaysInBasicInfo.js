    ...
    <View>
      <EditEventItem
        ...
        <BasicInfoItem
                    ...
                    value={currentUserData?.birthday ? moment(currentUserData.birthday * 1000).format('YYYY') : '-'}
                />

        <BasicInfoItem
                    title={strings.height}
                    value={currentUserData?.height ? `${currentUserData?.height} cm` : '-'}
                />

        <BasicInfoItem
                    title={strings.weight}
                    value={currentUserData?.weight ? `${currentUserData?.weight} kg` : '-'}
                />

        <BasicInfoItem
                    title={strings.mostUsedFoot}
                    value={currentUserData?.most_used_foot ? currentUserData?.most_used_foot : '-'}
                />
        <BasicInfoItem
  ...
  const onSavePress = () => {
    if (editModalType === strings.basicinfotitle) {
      ...
      const params = {
        ...
        birthday: userData.birthday === birthdayText ? userData.birthday : birthdayText,
        height: userData?.height ?? '',
        weight: userData.weight ?? '',
        most_used_foot: userData?.most_used_foot,
      }
      onSave(params).then(() => closeEditPlayInModal()).finally(() => setLoading(false))
    }
  }
