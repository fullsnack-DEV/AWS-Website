...
const EditPlaysInModal = ({
  ...
  useEffect(() => {
    ...
    setLocationData({ latitude: registerSport?.latitude, longitude: registerSport?.longitude })
  }, [registerSport])
  const onSavePress = () => {
    if (editModalType === strings.homePlaceTitle) {
      ...
      if (editSportIndex !== -1) {
        ...
        registered_sports[editSportIndex].longitude = locationData?.longitude ?? 0.0;
        registered_sports[editSportIndex].latitude = locationData?.latitude ?? 0.0;
        onSave({ registered_sports }).then(() => closeEditPlayInModal()).finally(() => setLoading(false))
      }
    }
  }
  return (
    <PlayInEditModal
      ...
      <ModalLocationSearch
            ...
            onSelect={(location) => {
              ...
              setHomePlace(city);
              setLocationData({ latitude: location?.latitude ?? 0.0, longitude: location?.longitude ?? 0.0 })
            }}
        />
      {editModalType === strings.homePlaceTitle ? (
        <TouchableOpacity onPress={() => setSearchLocationModal(true)} style={styles.containerStyle}>
