...
const AddSetGameModal = ({
  ...
  const getAfterSuchMinuitesTime = (date, numMinuite, isIncreased = true) => {
    ...
    return new Date(
      dummyDate.setMinutes(
        dummyDate.getMinutes() + (isIncreased ? numMinuite : -numMinuite),
      ),
    );
  };
  const [startsDate, setStartsDate] = useState(new Date());
  const [endsDate, setEndsDate] = useState(
    getAfterSuchMinuitesTime(new Date(), 1),
  );
      ...
      <SafeAreaView
        ...
        <ScrollView style={styles.contentContainer}>
          ...
          <View style={{padding: 15, flex: 1}}>
            ...
            {/* Starts */}
            <StartsEndComponent
              onPress={() => toggleDateTimePicker(1)}
              title={'Starts'}
              date={startsDate}
            />
            {/* Date Time Picker */}
...
const StartsEndComponent = ({title, date, onPress}) => (
  <TouchableOpacity
    onPress={onPress}
    activeOpacity={0.7}
