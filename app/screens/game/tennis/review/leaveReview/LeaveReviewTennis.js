...
import UserReview from '../../../../../components/game/tennis/review/leaveReview/UserReview';
  ...
  const LeaveReviewTennis = ({ navigation, route }) => {
    ...
    const [reviewsData, setReviewsData] = useState(currentForm === 1 ? {
      ...
      team_id: route?.params?.gameData?.home_team?.group_id || route?.params?.gameData?.home_team?.user_id,
      comment: '',
      attachments: [],
      tagged: [],
      ...
      team_id: route?.params?.gameData?.away_team?.group_id || route?.params?.gameData?.home_team?.user_id,
      comment: '',
      attachments: [],
      tagged: [],
    ...
    const patchOrAddReview = (data) => {
      if (route?.params?.gameReviewData) {
        ...
        setLoading(true);
        addGameReview(route?.params?.gameData?.game_id, data, authContext)
          .then(() => {
            setLoading(false);
            navigation.goBack();
          })
    ...
    const uploadMediaForTeamA = () => {
      if (reviewsData?.attachments?.length) {
        ...
        setReviewsData({ ...obj })
        patchOrAddReview(obj)
        })
      } else {
        patchOrAddReview(reviewsData)
      }
    }
