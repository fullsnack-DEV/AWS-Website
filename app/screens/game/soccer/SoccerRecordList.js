...
export default function SoccerRecordList({route}) {
  ...
  const getTeamName = (group_id) => {
    const singleTeamData =
      teamData?.home_team?.group_id?.toString() === group_id?.toString()
        ? teamData?.home_team
        : teamData?.away_team;
  ...
  const loadAtOnce = async () => {
    ...
    if (gameId) {
      ...
      getGameMatchRecords(gameId, authContext)
        .then((res) => {
  ...
  const onSwipeRowItemPress = (key, item) => {
    if (key === 'edit') {
      ...
      if (item?.verb === 'start') {
        ...
        setFullScreenLoading(true);
        deleteGameRecord(gameData?.game_id, item?.record_id, authContext)
          .then(() => {
  ...
  const onSelectEditDate = (date) => {
    ...
    setTimeout(() => {
      ...
      patchGameRecord(
        ...
        {timestamp, verb: selectedRecord?.verb},
        authContext,
      )
        .then(() => {
          refreshGameAndRecords(gameData?.game_id);
  ...
  const renderSoccerMatchRecords = useCallback(
    ({item}) => {
        ...
        <View style={styles.leftView}>
          ...
          <Text
            style={{
              ...styles.leftText,
              color:
                gameData?.away_team_goal !== gameData?.home_team_goal &&
