...
export default function EditLineUpCoachScreen({ navigation, route }) {
  ...
  useEffect(() => {
    if (route && route.params && route.params.gameObj && route.params.selectedTeam) {
      getLineUpOfTeams(route.params.selectedTeam === 'home' ? route.params.gameObj.home_team.group_id : route.params.gameObj.away_team.group_id, route.params.gameObj.game_id);
    }
  }, []);
